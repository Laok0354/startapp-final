...
const SideNavbar: React.FC<SideNavbarProps> = ({ page } : SideNavbarProps) => {
    ...
    return (
      <nav className={`fixed top-0 bg-black h-full border-r-2 border-[#1C1C1C] drop shadow-lg block transition-all duration-700 ${isOpen ? "w-[250px]" : "w-[100px]"}`}>
        <div className="flex justify-end items-baseline">
          ...
          <button className="flex flex-row text-[20px]">
            <Image className= ''
                ...
                height= {25}
            />
            <Link href="/login" className={`pl-3 transition-all ${isOpen ? "duration-1000" : "text-transparent duration-200"}`}>Projects</Link>
            </button>
          </div>
